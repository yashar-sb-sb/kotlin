...
import org.jetbrains.kotlin.fir.resolve.samResolver
...
private fun Candidate.prepareExpectedType(session: FirSession, argument: FirExpression, parameter: FirValueParameter): ConeKotlinType {
    val expectedType =
        getExpectedTypeWithSAMConversion(session, argument, parameter)
            ?: argument.getExpectedType(session, parameter/*, LanguageVersionSettings*/)
...
private fun Candidate.getExpectedTypeWithSAMConversion(
    ...
    if (!session.samResolver.shouldRunSamConversionForFunction(firNamedFunction)) return null
    ...
    val candidateExpectedType = argument.getExpectedType(session, parameter)
    ...
    return session.samResolver.getFunctionTypeForPossibleSamType(candidateExpectedType) ?: return null
}
